...
class ClientApp:
    ...
    def _start_read_loop(self):
        try:
            ...
            log.debug("caught exception, exiting")
        finally:
            self.sel.close()

    def start_connection():
        self.addr = (self.host, self.port)
        log.debug(f"starting connection to {addr}")
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.setblocking(False)
        self.sock.connect_ex(addr)
        event = threading.Event()
        read_thread = threading.Thread(target=_start_read_loop, 
                                       daemon=True, args=(e))
        read_thread.start()
        event.set()
...
def main():
    ...
    request = client.create_request('read_table', 'organization')
